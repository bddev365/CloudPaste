  ...
  >
    ...
    <div class="card">
      ...
      <div v-if="activeTab === 'paste'">
        ...
        <div class="markdown-toggle">
            ...
            <label for="markdown-toggle">Enable Markdown</label>
        </div>
        ...
        <div class="input-group">
          <label>Custom Link Suffix (Optional)</label>
          <input 
            type="text" 
            v-model="customId"
          ...
          <small v-if="files.length > 1" style="color: #666;">
            Custom links are not supported for multiple file uploads
          </small>
        </div>
        <div class="input-group">
          <label>Maximum Views (0 for unlimited)</label>
          <input 
            type="number" 
            v-model="maxViews"
            min="0"
            placeholder="0"
            title="Set the number of times the share can be opened, 0 or leave blank for unlimited"
          >
        </div>
        </div>

        <button class="btn" @click="submitPaste">Create Share</button>
      </div>
      ...
      <div v-if="activeTab === 'file'">
        ...
        <div class="file-list" v-if="files.length">
          ...
          <div v-for="(file, index) in files" :key="index" class="file-item">
            ...
            <div class="actions">
              <button class="btn" @click="removeFile(index)">Remove</button>
            </div>
          </div>
        </div>
        ...
        <div class="settings">
          ...
          <div class="input-group">
            <label>Password Protection</label>
            <input type="password" v-model="password" placeholder="Optional" autocomplete="new-password">
          </div>
          <div class="input-group">
            <label>Expiration Time</label>
            <select v-model="expiresIn">
          ...
          <!-- 添加可打开次数输入框 -->
          <div class="input-group">
            <label>Maximum Downloads (0 for unlimited)</label>
            <input 
              type="number" 
              v-model="maxViews"
              min="0"
      ...
      <div v-if="result" class="result">
        <div v-if="result.type === 'paste'">
          ...
          <div class="link">
            ...
            <div class="link-actions">
              <button class="btn" @click="copyUrl(result.url)">Copy Link</button>
              <button class="qr-btn" @click="showQR(result.url)" title="Show QR Code">
        ...
        <div v-else>
          ...
          <div v-for="file in result.files" :key="file.url" class="link">
            ...
            <div class="link-actions">
              <button class="btn" @click="copyUrl(file.url)">Copy Link</button>
              <!-- 修改直链按钮，点击复制直接下载链接 -->
              <button 
                class="qr-btn" 
              ...
              <button class="qr-btn" @click="showQR(file.url)" title="Show QR Code">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <path d="M3 3h6v6H3zM15 3h6v6h-6zM3 15h6v6H3z"/>
                  <path d="M15 15h2v2h-2zM19 15h2v2h-2zM15 19h2v2h-2zM19 19h2v2h-2z"/>
                </svg>
    ...
    <div class="admin-panel">
      ...
      <div v-if="showAdminLogin" class="admin-login">
        ...
        <div class="actions">
          <button class="btn" @click="adminLogin">OK</button>
          <button class="btn" @click="showAdminLogin = false">Cancel</button>
        </div>
      </div>
      ...
      <div v-if="isAdmin && showAdminPanel" class="admin-content">
        <div class="admin-header">
          ...
          <div class="admin-controls">
            <button 
              ...
              title="Allow/Disallow Text Upload"
            >
              Text Upload: {{ allowTextUpload ? 'On' : 'Off' }}
            </button>
            <button 
              class="control-btn" 
              :class="{ active: allowFileUpload }"
              @click="toggleFileUpload"
              title="Allow/Disallow File Upload"
            >
        ...
        <div class="storage-info">
          <h4 style="margin: 0 0 0.5rem 0;">R2 Storage Usage</h4>
          ...
          <div class="storage-details">
            ...
            <span>Total: {{ formatStorageSize(storageInfo.total) }}</span>
            <span>Usage: {{ storageInfo.percentage.toFixed(1) }}%</span>
          </div>
        </div>
          ...
          <div v-for="share in filteredShares" :key="share.id" class="share-item">
            <div class="title">
              ...
              <a 
                ...
                target="_blank" 
                  class="title-link"
                :title="'Click to open' + (share.type === 'paste' ? ' text' : ' file') + ' share'"
              >
                <span class="title-text">
                {{ share.type === 'paste' ? 'Text Share' : 'File Share' }}
                  <span v-if="share.hasPassword" class="lock-icon" title="Password Protected">🔒</span>
                </span>
              </a>
              </div>
              <div class="title-icons">
                ...
                <!-- 二维码按钮 -->
                <button class="icon-btn" @click="showQR(share.url)" title="Show QR Code">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="M3 3h6v6H3zM15 3h6v6h-6zM3 15h6v6H3z"/>
                    <path d="M15 15h2v2h-2zM19 15h2v2h-2zM15 19h2v2h-2zM19 19h2v2h-2z"/>
                  </svg>
            ...
            <div class="info">
              <div>ID: {{ share.id }}</div>
              <div>Created At: {{ formatDate(share.createdAt) }}</div>
              <div>Expires At: {{ share.expiresAt ? formatDate(share.expiresAt) : 'Never Expires' }}</div>
              <div v-if="share.type === 'file'">File Name: {{ share.filename }}</div>
              <!-- 根据分享类型显示不同的文本 -->
              <div v-if="share.maxViews > 0">
                {{ share.type === 'paste' ? 'Remaining Views' : 'Remaining Downloads' }}: 
                {{ share.maxViews - share.viewCount }}
                ({{ share.type === 'paste' ? 'Viewed' : 'Downloaded' }} {{ share.viewCount }} times)
              </div>
            </div>
            <div class="actions">
              <button class="btn" @click="copyUrl(share.url)">Copy Link</button>
              <button class="btn" @click="showChangePassword(share)">Change Password</button>
              <button class="btn delete-btn" @click="confirmDelete(share)">Delete</button>
            </div>
          </div>
        </div>
        ...
        <div v-if="showDeleteConfirm" class="confirm-dialog">
            <div class="confirm-content">
                ...
                <p>Are you sure you want to delete this {{ deleteTarget?.type === 'paste' ? 'text' : 'file' }} share?</p>
                <p class="warning">This action cannot be undone!</p>
                <div class="confirm-actions">
                    <button class="btn" @click="executeDelete">Delete</button>
    ...
    <div v-if="showPasswordDialog" class="confirm-dialog">
      <div class="confirm-content">
        ...
        <p>{{ passwordTarget?.type === 'paste' ? 'Text' : 'File' }} Share: {{ passwordTarget?.id }}</p>
        <div class="input-group">
          <label>New Password (Leave blank to remove password protection)</label>
          <input 
            type="password" 
            v-model="newPassword"
        ...
        <div class="confirm-actions">
          <button class="btn" @click="changePassword">OK</button>
          <button class="btn cancel" @click="showPasswordDialog = false">Cancel</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showQRCode" class="qr-dialog" @click.self="showQRCode = false">
      <div class="qr-content">
        ...
        <div class="qr-actions">
          <button class="btn" @click="downloadQR">Download QR Code</button>
          <button class="btn" style="background: var(--btn-secondary-bg)" @click="showQRCode = false">Close</button>
        </div>
      </div>
    </div>
  </div>
